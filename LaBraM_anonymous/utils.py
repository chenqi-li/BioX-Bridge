...
class TensorboardLogger(object):
    ...
    def update_figure(self, head='images', step=None, **kwargs):
        for k, v in kwargs.items():
            if v is None:
                continue
            self.writer.add_figure(head + "/" + k, v, self.step if step is None else step)
...
class DREAMERLoader(torch.utils.data.Dataset):
    def __init__(self, root, files, vad, threshold, sampling_rate=128):
        self.root = root
        self.files = files
        ...
        self.vad = vad
        self.files = [file for file in files if file.startswith("subject_1_")]
    ...
    def __getitem__(self, index):
        ...
        # if self.sampling_rate != self.default_rate:
        #     X = resample(X, 10 * self.sampling_rate, axis=-1)
        Y = 1 if sample[self.vad] >= self.threshold else 0
        X = torch.FloatTensor(X)
        return X, Y
...
def prepare_ISRUC_dataset(root, args):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    # prepare training and test data loader
    train_dataset = ISRUCLoader(os.path.join(root, "train"), train_files)
    test_dataset = ISRUCLoader(os.path.join(root, "test"), test_files)
    val_dataset = ISRUCLoader(os.path.join(root, "val"), val_files)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset

def prepare_DREAMER_dataset(root, args):
    # set random seed
    seed = 12345
    np.random.seed(seed)
    ...
    # prepare training and test data loader
    train_dataset = DREAMERLoader(os.path.join(root, "train"), train_files, args.vad, args.threshold)
    test_dataset = DREAMERLoader(os.path.join(root, "test"), test_files, args.vad, args.threshold)
    val_dataset = DREAMERLoader(os.path.join(root, "val"), val_files, args.vad, args.threshold)
    print(len(train_files), len(val_files), len(test_files))
    return train_dataset, test_dataset, val_dataset
...
def get_metrics(output, target, metrics, is_binary, threshold=0.5):
    if is_binary:
        ...
        binary_metrics = [metric for metric in metrics if metric in ['roc_auc', 'pr_auc']]
        multiclass_metrics = [metric for metric in metrics if metric not in ['roc_auc', 'pr_auc']]
        results = multiclass_metrics_fn(
            target, output, metrics=multiclass_metrics
        )
        if len(binary_metrics) > 0:
            outputs_tensor = torch.from_numpy(output.astype(np.float32))
            probs_tensor = F.softmax(outputs_tensor, dim=1)
            probs = probs_tensor.numpy()
            pos_probs = probs[:, 1]
            results_binary = binary_metrics_fn(target, pos_probs, metrics=binary_metrics)
