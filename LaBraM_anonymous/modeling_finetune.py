...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None, return_attention=False, return_qkv=False):
        # print(f'We got this as input: {x[0,0,:10]}') # DEBUG FORWARD PREHOOK
        if return_attention:
...
class NeuralTransformer(nn.Module):
    def __init__(self, EEG_size=1600, patch_size=200, in_chans=1, out_chans=8, num_classes=1000, embed_dim=200, depth=12,
        ...
        self.time_embed = nn.Parameter(torch.zeros(1, 30, embed_dim), requires_grad=True) # increase the acceptable token size
...
class NeuralTransformerECGFM968(NeuralTransformer):
    def __init__(self, *args, **kwargs):
        # Call the parent class (NeuralTransformer) constructor
        super().__init__(*args, **kwargs)

        # Manage directories
        fairseq_signals_root = '/users/anonymous/fairseq-signals'
        ...
        _ = hf_hub_download(
            ...
            local_dir=os.path.join(os.getcwd(), 'ckpts'),
        )
        _ = hf_hub_download(
            repo_id='wanglab/ecg-fm-preprint',
        ...
        self.ecg_fm = build_model_from_checkpoint(
            checkpoint_path=os.path.join(os.getcwd(), 'ckpts/mimic_iv_ecg_physionet_pretrained.pt')
        )

        self.ecg_fm.eval()
        print(f'ecg_fc {self.ecg_fm}')

        self.head = nn.Linear(200+768, self.num_classes) if self.num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        input_size = x.size()
        reshaped_x = x.view(input_size[0], input_size[1], -1)[:,:12,:]
        ecgfm_features = self.ecg_fm(source=reshaped_x, padding_mask=None, mask=False, features_only=False, mask_indices=None)
        # print(f'ecgfm_features {ecgfm_features["features"].size()}')
        ecgfm_features = ecgfm_features['features'].mean(dim=1)
        # print(f'ecgfm_features {ecgfm_features.size()}')
        x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
        # print(f'LaBraM output: {x.size()}')
        concatenated = torch.cat((x, ecgfm_features), dim=1)
        # print(f'concatenated {concatenated.size()}')
        x = self.head(concatenated)
        return x

class NeuralTransformerRand968(NeuralTransformer):
    def __init__(self, *args, **kwargs):
        # Call the parent class (NeuralTransformer) constructor
        super().__init__(*args, **kwargs)
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        input_size = x.size()
        reshaped_x = x.view(input_size[0], input_size[1], -1)[:,:12,:]
        ecgfm_features = torch.randn(input_size[0], 768)
        ecgfm_features = ecgfm_features.to(x.device)
        # print(f'ecgfm_features {ecgfm_features.size()}')
        x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
        # print(f'LaBraM output: {x.size()}')
        concatenated = torch.cat((x, ecgfm_features), dim=1)
...
class NeuralTransformerECGFM400(NeuralTransformer):
    def __init__(self, *args, **kwargs):
        # Call the parent class (NeuralTransformer) constructor
        super().__init__(*args, **kwargs)
        ...
        self.head = nn.Linear(200+200, self.num_classes) if self.num_classes > 0 else nn.Identity()
    ...
    def forward(self, x, input_chans=None, return_patch_tokens=False, return_all_tokens=False, **kwargs):
        ...
        # print(f'ecgfm_features {ecgfm_features["features"].size()}')
        ecgfm_features = ecgfm_features['features'].mean(dim=1)
        ecgfm_features = F.adaptive_avg_pool1d(ecgfm_features.unsqueeze(1), 200).squeeze(1)
        # print(f'ecgfm_features {ecgfm_features.size()}')
        x = self.forward_features(x, input_chans=input_chans, return_patch_tokens=return_patch_tokens, return_all_tokens=return_all_tokens, **kwargs)
        # print(f'LaBraM output: {x.size()}')
        concatenated = torch.cat((x, ecgfm_features), dim=1)
...
@register_model
def labram_base_patch200_200_ecgfm968(pretrained=False, **kwargs):
    model = NeuralTransformerECGFM968(
        patch_size=200, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def labram_base_patch200_200_rand968(pretrained=False, **kwargs):
    model = NeuralTransformerRand968(
        patch_size=200, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def labram_base_patch200_200_ecgfm400(pretrained=False, **kwargs):
    model = NeuralTransformerECGFM400(
        patch_size=200, embed_dim=200, depth=12, num_heads=10, mlp_ratio=4, qk_norm=partial(nn.LayerNorm, eps=1e-6), # qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    model.default_cfg = _cfg()
    return model
