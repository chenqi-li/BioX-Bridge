...
def get_args():
    ...
    parser.add_argument('--ft_layers', nargs='+', required=False)

    # Dataset parameters
    parser.add_argument('--vad', type=str, help='DREAMER: valence, arousal, dominance')
    parser.add_argument('--threshold', type=float, help='DREAMER: threshold for low/high valence')
...
def get_dataset(args):
    if args.dataset == 'TUAB':
        train_dataset, test_dataset, val_dataset = utils.prepare_TUAB_dataset("/data/anonymous/TUAB/v3.0.1/edf/processed")
        ...
        train_dataset, test_dataset, val_dataset = utils.prepare_TUEV_dataset("/data/anonymous/TUEV/v2.0.1/edf/processed")
        ...
        train_dataset, test_dataset, val_dataset = utils.prepare_DREAMER_dataset("/data/anonymous/DREAMER/processed_eeg128hz50overlap200windowFalsezstd", args)
        ch_names = None
        args.nb_classes = 1
        metrics = ["pr_auc", "roc_auc", "accuracy", "balanced_accuracy"]
    elif args.dataset == 'ISRUC':
        train_dataset, test_dataset, val_dataset = utils.prepare_ISRUC_dataset("/data/anonymous/ISRUC_S1/processed_eeg200hz_ecgfmhubertecg_eegfmlabram_5classes", args)
        ch_names = None
        args.nb_classes = 5
        metrics = ["accuracy", "balanced_accuracy", "cohen_kappa", "f1_weighted"]
    elif args.dataset == 'ISRUC_binary':
        train_dataset, test_dataset, val_dataset = utils.prepare_ISRUC_dataset("/data/anonymous/ISRUC_S1/processed_eeg200hz_ecgfmecgfm_eegfmlabram_2classes", args)
        ch_names = None
        args.nb_classes = 2
        metrics = ["accuracy", "balanced_accuracy", "cohen_kappa", "f1_weighted"]
...
def main(args, ds_init):
    ...
    for name, param in model.named_parameters():
        ...
        # if any(substring in name for substring in ['fc_norm.weight', 'fc_norm.bias', 'head.weight', 'head.bias']) or args.ft_layers == ['all']:
        if any(substring in name for substring in args.ft_layers) or args.ft_layers == ['all']:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            train_stats = evaluate(data_loader_train, model, device, header='Train:', ch_names=ch_names, metrics=metrics, is_binary=args.nb_classes == 1)
            print(f"Accuracy of the network on the {len(dataset_test)} test EEG: {train_stats['accuracy']:.2f}%")
            ...
            if log_writer is not None:
                for key, value in train_stats.items():
                    if key == 'accuracy':
                        log_writer.update(accuracy=value, head="train", step=epoch)
                    elif key == 'balanced_accuracy':
                        log_writer.update(balanced_accuracy=value, head="train", step=epoch)
                    elif key == 'f1_weighted':
                        log_writer.update(f1_weighted=value, head="train", step=epoch)
                    elif key == 'pr_auc':
                        log_writer.update(pr_auc=value, head="train", step=epoch)
                    elif key == 'roc_auc':
                        log_writer.update(roc_auc=value, head="train", step=epoch)
                    elif key == 'cohen_kappa':
                        log_writer.update(cohen_kappa=value, head="train", step=epoch)
                    elif key == 'loss':
                        log_writer.update(loss=value, head="train", step=epoch)
                    elif key == 'cm_percent':
                        plt.figure(figsize=(12, 7))
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_percent=ax.get_figure().get_figure(), head="train", step=epoch)
                    elif key == 'cm_abs':
                        plt.figure(figsize=(12, 7))
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_abs=ax.get_figure().get_figure(), head="train", step=epoch)
                ...
                for key, value in val_stats.items():
                    if key == 'accuracy':
                        ...
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_percent=ax.get_figure().get_figure(), head="val", step=epoch)
                    elif key == 'cm_abs':
                        plt.figure(figsize=(12, 7))
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_abs=ax.get_figure().get_figure(), head="val", step=epoch)
                for key, value in test_stats.items():
                    if key == 'accuracy':
                        ...
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_percent=ax.get_figure().get_figure(), head="test", step=epoch)
                    elif key == 'cm_abs':
                        plt.figure(figsize=(12, 7))
                        ax = sn.heatmap(value, annot=True, fmt='.2f')
                        ax.set(xlabel='Predicted Label', ylabel='True Label')
                        log_writer.update_figure(cm_abs=ax.get_figure().get_figure(), head="test", step=epoch)
        ...
        if args.output_dir and utils.is_main_process():
            ...
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
                f.write(json.dumps(log_stats, default=dataframe_to_dict, ensure_ascii=False) + "\n")
...
if __name__ == '__main__':
    ...
    if opts.output_dir:
        ...
        Path(os.path.join(opts.output_dir,'tensorboard')).mkdir(parents=True, exist_ok=True)
    
    # Initialize the print_output.txt and redirect all prints to the text file, will be restored to stdout later
    output_file_path = os.path.join(opts.output_dir, "print_output.txt")
