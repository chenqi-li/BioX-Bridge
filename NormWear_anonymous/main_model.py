...
class NormWearModel(nn.Module):
    def __init__(
        ...
        if len(weight_path) > 0:
            try:
                ...
                if 'normwear_last_checkpoint-15470-correct' in weight_path:
                    self.backbone.load_state_dict(stat_dict)
                else:
                    new_state_dict = {
                        k.replace("backbone.", "", 1): v for k, v in stat_dict.items()
    ...
    def get_embedding(self, sample_data, sampling_rate=65, device=torch.device('cuda')):
        # data: [bs, nvar, 3, L, n_mels]
        out = self.backbone.get_signal_embedding(sample_data.to(device), hidden_out=False, device=device) # bn, nvar, P, E
